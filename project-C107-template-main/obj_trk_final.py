...
while True:
    ...
    #checking if the first frame is being read 
    check,img = video.read()
    if not check:
        break
    #success : update i.e initialise the tracker
    success,bbox = tracker.update(img)
    
    #if tracker is initialised, call the drawbox i.e storing bbox info
    if success:
        drawBox(img,bbox)
    
    
    #show the tracked goal and all
    goal_track(img,bbox)
